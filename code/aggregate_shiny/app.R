...
server <- function(input, output) {
  ...
  output$barPlot <- renderPlotly({
    ...
    p <- finished_data %>%
      ...
      group_by(!!sym(label)) %>%
      count() %>%
      mutate({{label}} := as.numeric(as.character(!!sym(label)))/100) %>% # This weird syntax is necessary because R treats the dynamic label as a string, rather than a column name
      ggplot() +
